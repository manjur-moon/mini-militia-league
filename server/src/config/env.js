...
const envSchema = z
  .object({
    ...
    OCR_PROVIDER: ocrProviderSchema.default("disabled"),

    OCR_FALLBACK_PROVIDER: ocrProviderSchema.default("disabled"),
    ...
    PADDLE_OCR_URL: optionalUrl,

    PADDLE_OCR_TIMEOUT_MS: z.coerce
      .number()
      .int()
      .min(1_000)
      .max(180_000)
  ...
  .superRefine((value, context) => {
    ...
    const configuredCloudinaryCredentials =
      cloudinaryCredentials.filter(Boolean).length;

    if (
      configuredCloudinaryCredentials > 0 &&
      configuredCloudinaryCredentials < cloudinaryCredentials.length
    ) {
      context.addIssue({
        code: "custom",
    ...
    const selectedOCRProviders = new Set([
      value.OCR_PROVIDER,
      value.OCR_FALLBACK_PROVIDER,
    ]);

    if (
      !isTestRuntime &&
      selectedOCRProviders.has("google-vision") &&
      ...
      context.addIssue({
        ...
        message:
          "GOOGLE_VISION_API_KEY is required when Google Vision is configured as an OCR provider.",
      });
    }

    if (!isTestRuntime && selectedOCRProviders.has("gemini") && !value.GEMINI_API_KEY) {
      context.addIssue({
        code: "custom",
        path: ["GEMINI_API_KEY"],
        message:
    ...
    if (
      !isTestRuntime &&
      selectedOCRProviders.has("paddleocr") &&
      !value.PADDLE_OCR_URL
    ) {
      context.addIssue({
        code: "custom",
        path: ["PADDLE_OCR_URL"],
        message:
          "PADDLE_OCR_URL is required when PaddleOCR is configured as an OCR provider.",
      });
    }

    if (
      value.OCR_FALLBACK_PROVIDER !== "disabled" &&
      value.OCR_FALLBACK_PROVIDER === value.OCR_PROVIDER
    ) {
      context.addIssue({
        code: "custom",
        path: ["OCR_FALLBACK_PROVIDER"],
        message: "The fallback OCR provider must differ from the primary provider.",
      });
    }

    if (!isTestRuntime && value.AI_PROVIDER === "openai" && !value.OPENAI_API_KEY) {
      context.addIssue({
        code: "custom",
        path: ["OPENAI_API_KEY"],
...
if (sameSite === "none" && !isProduction && !isTestRuntime) {
  throw new Error(
...
if (!clientOrigins.length) {
  throw new Error("CLIENT_ORIGINS must contain at least one valid origin.");
}

const ocrColumnOrder = parsed.OCR_RESULT_COLUMN_ORDER.split(",").map((item) =>
  item.trim(),
);
...
export const env = Object.freeze({
  ...
  paddleOcrUrl: parsed.PADDLE_OCR_URL
    ? parsed.PADDLE_OCR_URL.replace(/\/+$/, "")
    : null,

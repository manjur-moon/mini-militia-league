...
describe("league date utilities", () => {
  it("converts an explicit league date to local-noon canonical UTC", () => {
    expect(
      leagueDateToCanonicalInstant("2026-09-15", "Asia/Dhaka").toISOString(),
    ).toBe("2026-09-15T06:00:00.000Z");
  });

  it("converts legacy timestamp input to the same local calendar date without a 7 AM rollover", () => {
    expect(
      leagueDateFromTimestamp("2026-09-15T06:15:00+06:00", "Asia/Dhaka"),
    ).toBe("2026-09-15");
  ...
  it("preserves the old 7 AM classification only for historical migration", () => {
    expect(
      resolveLegacyLeagueDate("2026-09-15T00:15:00.000Z", "Asia/Dhaka"),
    ).toBe("2026-09-14");
  });
  ...
  it("rejects future league dates", () => {
    ...
    expect(() =>
      assertLeagueDateNotFuture("2026-09-20", "Asia/Dhaka"),
    ).toThrowError(
    ...
    expect(() =>
      assertLeagueDateNotFuture("2026-09-19", "Asia/Dhaka"),
    ).not.toThrow();
